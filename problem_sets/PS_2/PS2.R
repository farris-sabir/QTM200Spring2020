...
## part d
# After knowing that we have failed to reject the null hypothesis that our variables are dependent, we explore the variables' relation further
...
## part a
# The null hypothesis is that the average number of new/repaired drinking-water facilities in villages with reservation policy is equivalent to that in villages without reservation policy.
# The alternative hypothesis is that the average number of new/repaired drinking-water facilities in villages with reservation policy is not equivalent to that in villages without reservation policy.
...
ggplot(fruitfly, aes(lifespan)) + geom_histogram(binwidth = 5)
...
# hypotheses
# null hypothesis: there is no linear relationship between lifespan and thorax, meaning its beta or slope is equal to 0.
# alternative hypothesis: there is a linear relationship between lifespan and thorax, meaning its beta or slope is not equal to 0.

# testing the null hypothesis that beta = 0
stdv_lifespan_hat <- sqrt(sum(residf^2)/(length(fruitfly$lifespan)-2))
se_lifespan_hat <- stdv_lifespan_hat/sqrt(sum((fruitfly$thorax - thorax_bar)^2))
tf <- (beta_hatf - 0)/se_lifespan_hat
pf <- 2*pt(tf, df = (length(fruitfly$lifespan)-2), lower.tail=F)
pf
#p-value = 1.497*10^-15
...
# Because the beta of 144.333 is associated with a t-value of 9.152 and p-value of 1.497*10^-15, with an alpha of 0.05, we reject the null hypothesis.
# As a result, there is a significant linear relationship between thorax length and lifespan of fruitflies.
...
ci
# The confidence interval is 118.196 to 107.470.
...
thoraxi <- 0.8
lifespani <- alpha_hatf + beta_hatf*thoraxi
lifespani
